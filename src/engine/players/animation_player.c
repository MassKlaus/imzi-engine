...
AnimationPlayer Imzi_CreateAnimationPlayer(AssetManager2D *manager, int32_t animation_index)
{
...
{
    int32_t animation_index = Imzi_AssetManager2DGetAnimationByName(player->manager, name);
    Imzi_StartAnimationPlayer(player, animation_index);
}

void Imzi_StartAnimationPlayer(AnimationPlayer *player, int32_t animation_index)
{
...
void Imzi_AnimationPlayerRender(Imzi_Engine_Ptr engine, AnimationPlayer *player, vec2 position)
{
    ...
    SDL_FRect src = {animation->area.x + animation->frame_w * frame, animation->area.y, animation->frame_w, animation->frame_h};
    // SDL_FRect src = {0, 0, 64.0, 64.0};
    SDL_FRect dest = {position[0], position[1], animation->area.w, animation->area.h};
    // SDL_FRect dest = {0, 0, 64.0, 64.0};

    SDL_FlipMode flip = SDL_FLIP_NONE;

    if (player->h_flip) flip |= SDL_FLIP_HORIZONTAL;
    if (player->v_flip) flip |= SDL_FLIP_VERTICAL;
